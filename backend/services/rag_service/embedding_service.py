...
1. OpenVINO (Intel-optimized INT8 inference via optimum-intel)
2. PyTorch (default sentence-transformers backend)
3. Simple word embeddings (fallback when no ML framework available)
"""
import asyncio
import logging
import numpy as np
...
class LocalEmbeddingService:
    ...
    async def initialize(self) -> bool:
        ...
        Tries OpenVINO backend first (Intel-optimized INT8 inference on CPU),
        falls back to default PyTorch backend.
        """
        try:
            ...
            if self.model:
                ...
                logger.info(f"Embedding dimension: {self.embedding_dim}, backend: {self.backend}")
